...
import { pipeWithHelper, Result, Success, Failure } from './result';

describe('pipeWithHelper', () => {
    it('should apply the function to the success data if the result is of type "success"', () => {
        const curFun = vi.fn((data: number) => ({ type: 'success', successData: data * 2 }));
        ...
        const actualResult = pipeWithHelper(curFun, prevResult);
